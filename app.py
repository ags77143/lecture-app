...
def transcribe_audio(url, log):
    log.write("No captions found — downloading audio...")

    # Always upgrade yt-dlp first to get latest YouTube fixes
    log.write("Updating yt-dlp to latest version...")
    ...
    log.write("Downloading audio...")
    result = subprocess.run(cmd, capture_output=True, text=True)

    if result.returncode != 0:
        log.write(f"Download error: {result.stderr[-300:]}")
    ...
    if not audio_file:
        log.write("Audio download failed.")
        return None

    file_size = os.path.getsize(audio_file)
    log.write(f"Audio downloaded ({file_size // 1024} KB). Transcribing with Whisper...")
